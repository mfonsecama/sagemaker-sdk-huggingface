  def validate_version_or_image_args(framework_version, py_version):
    ...
    Raises:
        ValueError: if `framework_version['transfomers']` is None and `framework_version['datasets']` is None and `py_version` is
            not 3.
    """
    if (framework_version is None or py_version is not '3' or 'transformers' not in framework_version or 'datasets' not in framework_version) and image_uri is None:
        raise ValueError(
